import type { LayoutRouter } from '../layout/client.ts'
import type { PageNode, DocPage } from '../layout/types.ts'
import { groupItems } from '../layout/client.ts'
import type * as Reflect from '../reflect/types.ts'
import { labelOf } from '../naming.ts'

/**
 * Resolve a `{@link Name}` reference to the href written into the markdown.
 * The caller owns the final shape — the client prefixes the router base, the
 * llms.txt generator returns an absolute URL — so this module never has to know
 * where the site is served from.
 */
export type SlugOf = (name: string) => string | undefined

/**
 * The slice of a project the serializer needs. Structural, so both the client's
 * indexed `Project` and a bare `ProjectVersion` wrapper satisfy it — this module
 * is pure serialization of core types and must not depend on the UI layer.
 */
...
type Ctx = { project: DeclarationLookup; slugOf: SlugOf; inline: boolean; seen: Set<number>; router: LayoutRouter }
...
export const routeToMarkdown = (
  router: LayoutRouter,
  route: PageNode,
  project: DeclarationLookup,
  slugOf: SlugOf,
  opts: MarkdownOptions = {},
): string =>
  renderRoute(route, { project, slugOf, router, inline: !!opts.inlineMembers, seen: new Set() }, 1)

const renderRoute = (route: PageNode, ctx: Ctx, depth: number): string => {
  if (route.kind === 'page') return route.body.join('\n\n').trimEnd() + '\n'
  // A component page's body only exists once rendered, so the markdown export
  // carries its title and points at the module rather than inventing content.
  if (route.kind === 'component')
    return `${'#'.repeat(Math.min(depth, 6))} ${route.title}\n\n*rendered from \`${route.module}\`*\n`
...
const commentText = (text: string, target: string | undefined, slugOf: SlugOf): string => {
  ...
  return slug ? `[${text || target}](${slug})` : text
}
