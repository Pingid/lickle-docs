...
test('infers type parameters as type-parameter references', () => {
  ...
  expect(ret.type).toBe('external')
  expect((ret as Extract<T.Type<'reference'>, { type: 'external' }>).external).toBe('type-parameter')
})
...
test('infers enum members and widened enums as references', () => {
  ...
  expect((r as Extract<T.Type<'reference'>, { type: 'internal' }>).targetId).toBe(byName(idx, 'Color').id)

  const w = typeOf(idx, 'w') as T.Type<'reference'>
  expect(w.kind).toBe('reference')
  expect(w.name).toBe('Color')
  expect(w.type).toBe('internal')
})
