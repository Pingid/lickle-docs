...
const registerServiceWorker = async () => {
  if ('serviceWorker' in navigator) {
    try {
      const registration = await navigator.serviceWorker.register(new URL('sw.ts', import.meta.url), { type: 'module' })
...
async function mainNonStreaming() {
  ...
  const engine: webllm.MLCEngineInterface = await webllm.CreateServiceWorkerMLCEngine(selectedModel, {
...
async function mainStreaming() {
  ...
  const engine: webllm.ServiceWorkerMLCEngine = await webllm.CreateServiceWorkerMLCEngine(selectedModel, {
    initProgressCallback: initProgressCallback,
  })
